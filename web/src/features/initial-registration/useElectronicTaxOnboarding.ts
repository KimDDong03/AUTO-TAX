    ...
    async (
      ...
      if (matches.length === 0) {
        ...
        if (expiredMatches.length > 0) {
          throw new Error(
            `${customer.customerName} 고객의 발행 가능 공동인증서가 만료되어 자동 등록에서 제외했습니다. 갱신한 인증서를 다시 불러와 주세요.`
          );
        }
        throw new Error(`${customer.customerName} 고객의 발행 가능 공동인증서를 이 PC에서 다시 찾지 못했습니다.`);
      }
      if (matches.length > 1) {
        throw new Error(
          `${customer.customerName} 고객과 일치하는 발행 가능 공동인증서를 ${matches.length}건 발견해 자동 등록을 중단했습니다. 같은 CN 인증서가 여러 개이면 인증서 목록에서 확인 후 수동으로 등록하세요.`
        );
      }
