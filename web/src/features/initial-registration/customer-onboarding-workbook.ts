...
export function downloadCustomerOnboardingTemplate(
  ...
  const guideRows = [
    ...
    ["업로드 순서", "1) 로컬 헬퍼 실행 후 전자세금용 공동인증서 읽기 확인 2) 양식 다운로드 3) 발전소 시트에서 등록할 고객 행만 남기고 발전소명과 필요 시 인증서 비밀번호 입력 4) 양식 업로드 후 전자세금용 인증서 확인 결과와 고객 생성/갱신 가능 여부 확인 5) 고객 등록 반영 6) 전자세금용 인증서 등록 마무리"]
  ];
  const plantRows = [
