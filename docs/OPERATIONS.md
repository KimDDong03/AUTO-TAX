...
- Popbill certificate registration is helper-assisted but row-orchestrated by the web app in initial registration. The Popbill certificate-registration URL is short-lived, so the web app requests a fresh URL immediately before each selected customer calls the helper's single `/api/popbill/certificate-registration` route instead of pre-building a batch of URLs. The helper still owns the one-row browser/context lifetime, certificate-frame matching, and failure capture. Headless registrations use isolated temporary Chrome profiles; if headless ML4Web frame readiness fails or an expired-token signal appears, the web app gets a new URL and retries that row once in visible compatibility mode. The helper job route `/api/popbill/certificate-registration-jobs` remains available only for compatibility/diagnostics with already-fresh URLs. Password and certificate-selection failures remain actionable row failures. Frame readiness failures write both the ML4Web child frame HTML and the main certificate page HTML under the local Popbill debug artifact directory so the clicked page state can be diagnosed without rerunning immediately.
...
- Raw NPKI/P12/PFX files selected during manual customer add or initial registration must stay inside the browser-to-`127.0.0.1` helper boundary. Persist only extracted certificate metadata and customer fields; the helper may write/import selected initial-registration certificate material locally only to prepare bridge-backed HomeTax lookup.
