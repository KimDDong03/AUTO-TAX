...
- `web/src/features/customers/`
  ...
  - certificate-first customer add flow for one-stop customer creation, certificate linking, Popbill join, and certificate registration retries; its file/folder add path reuses the initial-registration candidate table pattern for multi-select, shared/per-row certificate passwords, spreadsheet-style paste, and row-level verification status
...
5. The UI keeps the checklist as the primary surface: the customer initial-registration step uses a compact active-step header and one candidate workspace rather than stacked status cards. The workspace shows only read/selected/check-result counts, a selected-row delete action, the shared password field, the selected-customer check action, and a table with a header select-all checkbox, row target checkbox, read-only customer-facing company name, optional per-certificate password, and row status. The candidate table owns its vertical scroll area so long certificate lists remain visibly scrollable inside the work surface. Routine status cards and generic success notices are hidden on this selection step.
6. During target check, file/folder-added candidates are prepared for the bridge-backed lookup path before any business-info lookup starts. The app first submits the selected upload-session certificates to `/api/certificates/import-upload-session` in chunks of up to 50 rows: NPKI pairs are copied into the standard LocalLow NPKI store, while `.p12`/`.pfx` files are imported through SecuKit NXS `CertManagement.importP12` using the entered certificate password. The import path warms SecuKit's normal HDD storage selection before `importP12`, then re-reads the bridge certificate list and swaps each upload-session candidate for the bridge-backed certificate index. If any selected upload-session certificate cannot be password-validated, imported into NPKI, or matched back to the bridge list, target check stops there and no SignGate/HomeTax business-info lookup is started for that run.
...
10. Initial-registration certificate registration keeps registration URL lifetime in the web layer but uses helper-owned batch execution for speed. Popbill certificate-registration URLs are one-time, short-lived links, so the UI prepares selected customer/certificate pairs without URLs, splits them into five-row windows, requests fresh URLs for the current window through `/api/customers/popbill/cert-urls`, then sends those already-fresh URLs to the helper job route `/api/popbill/certificate-registration-jobs` with explicit `direct` mode. The helper direct path reads the current HDD/NPKI certificate list from the local `127.0.0.1:42235` service, matches the selected certificate by serial/userDN or CN plus expiry, builds the same certificate payload that the Popbill ML4Web popup posts, exchanges the fresh popup token, and posts directly to `/__API_V1__/Taxinvoice/Preference/Certificate`. Operator-action failures such as wrong password, expired certificate, expired registration URL, or business-number mismatch stay failed. If a direct batch row fails with a structural/direct-path error, the UI requests a fresh URL and retries only that row through the single `/api/popbill/certificate-registration` compatibility route starting at `headless` mode so it does not repeat the failed direct attempt. If an expired-token signal appears, the row gets a fresh URL and retries from `direct`; frame-readiness failures can still escalate to visible compatibility mode. Successful rows are confirmed afterward through `/api/customers/popbill/cert-status-batch` instead of one customer at a time. `AUTO_TAX_POPBILL_HELPER_HEADLESS=0` still forces headed compatibility mode for PCs where ML4Web cannot work headlessly, and explicit `AUTO_TAX_POPBILL_HELPER_USER_DATA_DIR` keeps persistent-profile behavior for manual troubleshooting. Helper-owned certificate-registration jobs default to five concurrent rows and cap at five because local testing confirmed five simultaneous MagicLine4NX direct calls succeeded while six simultaneous calls can produce a loopback connection refusal.
11. Selected-customer check results are normalized into row-level review state before execution is enabled. Password failures keep the row selected, mark it as `needs_fix`, and show the correction inside that row's password cell area. The failed password is retained only in browser state so a changed password can move the row to `needs_recheck` instead of looking ready before the operator runs the check action again. The checklist provides `전체`, `확인 필요`, and `비밀번호` filters, and password problem rows reuse the main top action row for selected-row password application when several customers share the same corrected password. Missing business/address data is marked as `manual_required` and exposes only the fields needed to repair the row, such as business number, representative, and business address. The operator must press the check action again after corrections; stale previews are not treated as executable. Row-mapped errors are not duplicated as a long global list. The global review area shows compact issue counts and keeps only unmatched/global messages behind a collapsed detail view.
12. If business-info lookup returns taxpayer identity but no business or matching address even after the taxpayer-basic lookup path, the customer is not allowed through the one-click initial execution until the missing address/manual fields are supplied and the selected-customer check succeeds again. Address remains required for KEPCO mail matching, so rows without a matching address are blocked in the initial-registration review instead of being silently created as customers that cannot match mail later. When the HomeTax taxpayer-basic helper call failed, the row-level review message includes that lookup detail so operators can distinguish a true missing address from a lookup-path problem.
13. If the unified business-info lookup fails after it was attempted, initial registration surfaces that business-info/manual-information failure and does not fall through to the legacy SignGate renewal preflight path. SignGate preflight remains a renewal diagnostic path for renewal/payment operations. If a selected upload-session certificate cannot be imported or matched back to the bridge list, preview fails that row with an actionable message instead of silently using incomplete data.
14. Preview and commit still use the server customer-onboarding endpoints and async batch flow. After the operator confirms selected targets and every selected row is ready, the UI exposes a single initial-registration execution action. That action commits customer changes first, keeps polling the follow-up join runner until reflected customers are confirmed joined or a bounded wait expires, reloads the latest customer/certificate state, recalculates pending certificate-registration targets from that fresh payload, and then runs the helper-owned certificate-registration batch. The shared password input is only part of target selection/checking; execution progress is shown as customer reflection followed by certificate registration. When both customer reflection and certificate registration are complete, the step switches to a completion state with a next-step action that opens the first mail sync step.
...
- Customer management's one-stop customer add uses the same certificate source and selected-customer verification path as initial registration. Operators can add candidates from the bridge-backed "공동인증서 읽기" path, file selection, or folder selection; registered customer certificates and duplicate uploaded/NPKI copies are filtered before rows are shown. When the operator enters a shared or per-row password and clicks selected-customer check, the selected rows are converted to the same temporary onboarding workbook, then run through the initial-registration resolver so password validation, `.p12`/`.pfx` NPKI import, SignGate-first business-info lookup, HomeTax fallback, and row-level failure classification stay identical. File/folder candidates are not treated as executable until that check imports the certificate into the standard NPKI store and swaps the row to the bridge-readable certificate.
