...
11. Selected-customer check results are normalized into row-level review state before execution is enabled. Password failures keep the row selected, mark it as `needs_fix`, and show the correction inside that row's password cell area. Missing business/address data is marked as `manual_required` and exposes only the fields needed to repair the row, such as business number, representative, and business address. The operator must press the check action again after corrections; stale previews are not treated as executable. Generic warning text is shown only for unmatched messages that cannot be mapped to a row.
