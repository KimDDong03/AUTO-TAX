...
test("canEnableAutoIssueForCustomer prefers manual issue success evidence from app_logs", async () => {
  const calls: Array<{
    table: string;
    filters: Array<{ type: "eq" | "contains"; column: string; value: unknown }>;
    limit?: number;
  }> = [];
    ...
    from(table: string) {
      return {
        select() {
          const call = {
            table,
            filters: [] as Array<{ type: "eq" | "contains"; column: string; value: unknown }>,
          ...
          const query = {
            ...
            contains(column: string, value: unknown) {
              call.filters.push({ type: "contains", column, value });
              return query;
            },
            limit(limit: number) {
              call.limit = limit;
  ...
  Object.assign(store as object, {
    ...
    initialized: true,
    organizationId: "org-1",
    getManagedCustomerRowByLegacyId: async (customerId: number) => {
      assert.equal(customerId, 1);
      return { id: "customer-uuid" };
    }
  });

  const result = await store.canEnableAutoIssueForCustomer(1);
  ...
  assert.deepEqual(calls, [
    {
      table: "app_logs",
      filters: [
        { type: "eq", column: "organization_id", value: "org-1" },
        {
          type: "contains",
          column: "context_json",
          value: {
...
test("canEnableAutoIssueForCustomer falls back to issued drafts when manual issue log evidence is absent", async () => {
  const calls: Array<{
    table: string;
    filters: Array<{ type: "eq" | "contains"; column: string; value: unknown }>;
    limit?: number;
  ...
  assert.deepEqual(calls, [
    ...
    },
    {
      ...
      filters: [
        { type: "eq", column: "organization_id", value: "org-1" },
        { type: "eq", column: "managed_customer_id", value: "customer-uuid" },
        { type: "eq", column: "status", value: "issued" }
      ],
      limit: 1
    }
  ]);
