...
> Current canonical UI guide.
>
> Companion docs for the redesign effort:
> - `docs/UI_UX_REDESIGN_DESIGN_SYSTEM_V1.md`
> - `docs/UI_UX_REDESIGN_WORKING_PLAN.md`
>
> Until the redesign merge is complete, follow this file first, then use the companion docs for future-state direction and migration planning.
...
- 새 foundation/UI 코드에서는 `--color-*`, `--radius-*`, `--shadow-*`, `--font-*`, `--size-*`, `--z-*` 토큰을 우선 사용한다.
- 기존 `--bg`, `--primary`, `--radius-sm`, `--shadow-sm` 같은 토큰은 화면 분해가 끝날 때까지 호환성 alias로 유지한다.

### Color

- 배경: `--color-bg-canvas`, `--color-bg-surface`, `--color-bg-surface-subtle`
- 텍스트: `--color-text-default`, `--color-text-subtle`, `--color-text-muted`
- 주요 행동: `--color-brand-default`, `--color-brand-strong`, `--color-brand-subtle`
- 성공: `--color-success-default`, `--color-success-subtle`
- 경고: `--color-warning-default`, `--color-warning-subtle`
- 위험: `--color-danger-default`, `--color-danger-subtle`
- 경계선: `--color-border-default`, `--color-border-strong`
- 레거시 alias: `--bg`, `--surface`, `--text`, `--primary`, `--success`, `--warning`, `--danger`, `--border`
...
- 큰 카드: `--radius-3` (`--radius-lg` alias)
- 일반 카드/그룹: `--radius-2` (`--radius-md` alias)
- 버튼/작은 요소: `--radius-1` (`--radius-sm` alias)
- 그림자: `--shadow-1`, `--shadow-2` (`--shadow-sm`, `--shadow-md` alias)
...
기존 `chip`, `status`, `alert`는 호환 계층으로 유지하되, 새 shared code에서는 `StatusBadge`, `SectionMessage`, `EmptyState`를 우선 사용한다.
현재 `AppDialog`, `SetupPanel`은 badge 표시에 `StatusBadge`를 사용한다.
`인증서 관리`의 미연결 인증서 compact empty block은 `EmptyState`를 사용한다.
`인증서 관리`의 조치 필요 고객 상태 badge, 작업 상태 안내, 연결 전 empty block도 shared primitive로 우선 정리한다.
`고객 운영`의 상단 경고, 준비 상태 badge, 이력/목록 empty state도 shared primitive로 우선 정리한다.
